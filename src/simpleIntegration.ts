...
export class SimpleIntegration {
  ...
  private visualizeCesiumFrustum(cesiumCamera: any, frustum: any): void {
    ...
    try {
      ...
      console.log('🎯 FRUSTUM VIZ:', {
        ...
        childMeshes: this.frustumVisualization
          ? this.frustumVisualization.getChildMeshes().length
          : 0,
  ...
  private createBoundingVolumeWireframes(): void {
    ...
    console.log(
      `🔧 Creating bounding volume wireframes for ${selectedTiles.length} selected tiles`
    );
    ...
    selectedTiles.forEach((tile: any, index: number) => {
      ...
      console.log(
        `   [${index}] Depth ${tile._depth}: center=(${cesiumCenter.x.toFixed(0)}, ${cesiumCenter.y.toFixed(0)}, ${cesiumCenter.z.toFixed(0)}), radius=${(cesiumRadius / 1000).toFixed(1)}km`
      );
    });
  }
  ...
  private createFrustumWireframe(): void {
    ...
    const nearTL = nearCenter
      .add(camUp.scale(nearHalfHeight))
      .subtract(camRight.scale(nearHalfWidth));
    const nearTR = nearCenter.add(camUp.scale(nearHalfHeight)).add(camRight.scale(nearHalfWidth));
    const nearBL = nearCenter
      .subtract(camUp.scale(nearHalfHeight))
      .subtract(camRight.scale(nearHalfWidth));
    const nearBR = nearCenter
      .subtract(camUp.scale(nearHalfHeight))
      .add(camRight.scale(nearHalfWidth));
    ...
    const farBL = farCenter
      .subtract(camUp.scale(farHalfHeight))
      .subtract(camRight.scale(farHalfWidth));
    ...
    const frustumLines = MeshBuilder.CreateLines(
      'frustumWireframe',
      {
        points: points,
      },
