...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    function createReferenceObjects(scene: Scene) {
        ...
        const earthSphere = MeshBuilder.CreateSphere("earthSphere", {diameter: earthRadius * 2, segments: 64}, scene);
        ...
        const skyBarrier = MeshBuilder.CreateSphere("skyBarrier", {diameter: skyBarrierRadius * 2, segments: 32}, scene);
    ...
    function setupKeyboardControls(camera: UniversalCamera, scene: Scene, integration: any) {
        ...
        window.addEventListener('keydown', (event) => {
            ...
            if (key >= '1' && key <= '9') {
                ...
                const tileMeshes = scene.meshes.filter(m => m.name !== 'camera' && m.name !== 'earthSphere');
